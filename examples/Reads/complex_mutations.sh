...
: Now take reads from the genome
python reads.py --wg=${DATADIR}/chimera_complex.h5 --out=${DATADIR}/chimera_complex_reads  --paramfile=${DIR}/complex_genome_reads.json -v --fastq

: Do some alignment to check
bwa index ${DATADIR}/chimera.fa.gz
bwa mem -p ${DATADIR}/chimera.fa.gz ${DATADIR}/chimera_complex_reads.fastq > ${DATADIR}/chimera_complex_aligned.sam